...
pub fn sublist<T: PartialEq + Clone>(_first_list: &[T], _second_list: &[T]) -> Comparison {
    ...
    fn sublist_helper<T: PartialEq + Clone>(_short_list: &[T], _long_list: &[T]) -> Comparison {
        let (mut short_list_cursor, mut long_list_cursor) = (0, 0);
        let mut long_list_start = 0;
        let mut matched_count = 0;
        let (short_list_len, long_list_len) = (_short_list.len(), _long_list.len());

        while short_list_cursor < short_list_len && long_list_cursor < long_list_len {
